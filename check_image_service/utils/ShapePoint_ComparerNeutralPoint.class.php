...
class ShapePoint_ComparerNeutralPoint{
	...
	public static function compare($shapePointList1,$shapePointList2){
		...
		// 1-  mise à jour des histogrammes
		for($i = 0; $i < count($shapePointList1); $i ++) {
			$histogramm = ShapePoint_ComparerNeutralPoint::getHistogramm($shapePointList1[$i], $shapePointList1);
			$shapePointList1[$i]->setHistogram($histogramm);
				
		}
			
		for($i = 0; $i < count($shapePointList2); $i ++) {
			$histogramm = ShapePoint_ComparerNeutralPoint::getHistogramm($shapePointList2[$i], $shapePointList2);
			$shapePointList2[$i]->setHistogram($histogramm);
		}
		// 2-   Calcul de la matrice C
		$n1 = count($shapePointList1); $n2 = count($shapePointList2);
		if($n1<=$n2){
			$C = array();
			for($i = 0; $i < $n1; $i ++) {
				$CarrayLigne=array();
				for($j = 0; $j < $n2; $j ++) {
					$CarrayLigne[] = floor(ShapePoint_ComparerNeutralPoint::compareHistogram($shapePointList1[$i],$shapePointList2[$j]));
				}
				$C[]=$CarrayLigne;
			}
			for($i = $n1; $i < $n2; $i ++) {
				$CarrayLigne=array();
				for($j = 0; $j < $n2; $j ++) {
					$CarrayLigne[] = Constants::getDummyPointCost();
				}
				$C[]=$CarrayLigne;
			}
		}else{
	...
	/* $C = array(array(1,1,1,1,1,1,1,1),
	   ...
	   /* $C = array(array(9,12,9,6,9,6,9,10),
		...
		// 4 - Réduction des shapePointList en fonction des permutations
		/*echo "taille 1   " . $n1."\n";
		echo "taille 2    ". $n2."\n";*/
		if($n1<=$n2){
			...
			$tmp_shapePointList1 = array();
			$tmp_shapePointList2 = array();
			$tmp_permutation = array();
			for($i = 0; $i < $n1; $i ++){
				if($permutations[$i]["j"]<$n2){
					$tmp_permutation [] = $permutations[$i];
					$tmp_shapePointList1[] = $shapePointList1[$permutations[$i]["i"]];
				}
			}
			...
			for($i = 0; $i < $n2; $i ++){
				$tmp_shapePointList2[$i]=$shapePointList2[$tmp_permutation[$i]["j"]];
			}
			$shapePointList1 = $tmp_shapePointList1;
			$shapePointList2 = $tmp_shapePointList2;			
		}
		...
		for($i = 0; $i < count($shapePointList2); $i ++){
			$shapePointList3[] = ShapePoint_ComparerNeutralPoint::thinSplineTransformationFunction(
		...
		for($i = 0; $i < count($shapePointList3); $i ++) {
			$histogramm = ShapePoint_ComparerNeutralPoint::getHistogramm($shapePointList3[$i], $shapePointList3);
		...
		for($i = 0; $i < $tailleC; $i ++){
			$min = ShapePoint_ComparerNeutralPoint::compareHistogram($shapePointList1[$i], $shapePointList3[0]);
			$arg_min=0;
			for($j = 0; $j < $tailleC; $j ++){
				$temp = ShapePoint_ComparerNeutralPoint::compareHistogram($shapePointList1[$i], $shapePointList3[$j]);
				if($min > $temp){
					$min = $temp;
					$arg_min = $j;
				}
		...
		for($i = 0; $i < $tailleC; $i ++){
			$min = ShapePoint_ComparerNeutralPoint::compareHistogram($shapePointList1[0], $shapePointList3[$i]);
			$arg_min=0;
			for($j = 0; $j < $tailleC; $j ++){
				$temp = ShapePoint_ComparerNeutralPoint::compareHistogram($shapePointList1[$j], $shapePointList3[$i]);
				if($min > $temp){
					$min = $temp;
					$arg_min = $j;
				}
	...
	public static function reduce($shapePointList,$n){
		...
		for($i = 0; $i < $N; $i ++){
			...
			for($j = 0; $j < $n; $j ++){
				$dist_act = ShapePoint_ComparerNeutralPoint::distanceBetween($shapePointList[$i],$cluster_centers[$j]);
				if($dist_act<$distMin){
					$distMin = $dist_act;
					$jmin = $j;
				}
		...
		while($change==1){
			...
			for($i = 0; $i < $N; $i ++){
				$jmin = 0;$distMin = ShapePoint_ComparerNeutralPoint::distanceBetween($shapePointList[$i],$cluster_centers[0]);
				for($j = 0; $j < $n; $j ++){
					$dist_act = ShapePoint_ComparerNeutralPoint::distanceBetween($shapePointList[$i],$cluster_centers[$j]);
					if($dist_act<$distMin){
						$distMin = $dist_act;
	...
	public static function getHistogramm($shapePoint, $shapePointList){
		$Rintervall = ShapePoint_ComparerNeutralPoint::getRIntervall($shapePointList);
		...
		foreach ($shapePointList as $actual_shapePoint){	
			...
			//echo "\n********** index ajouté :".ShapePoint_ComparerNeutralPoint::getPinIndex($RPolar, $Teta,$Rintervall);
			$indice = ShapePoint_ComparerNeutralPoint::getPinIndex($RPolar, $Teta,$Rintervall);
			if($indice>=Constants::getRAreaNumberHistrogram()*Constants::getTetaAreaNumberHistogram()) {
				$indice = Constants::getRAreaNumberHistrogram()*Constants::getTetaAreaNumberHistogram()-1;
			}
			$histogramm[$indice] = $histogramm[$indice]+1;		
	...
	public static function getPinIndex($RPolar,$Teta,$Rintervall){
		...
		return Constants::getRAreaNumberHistrogram()*floor($temp)  +
				floor($Teta/ShapePoint_ComparerNeutralPoint::getTetaIntervall());
	}
	...
	public static function getRIntervall($shapePointList){
		//echo "R max de shapePointList".exp(log10(ShapePoint_ComparerNeutralPoint::getRmax($shapePointList)));
		return exp(log10(ShapePoint_ComparerNeutralPoint::getRmax($shapePointList)))/Constants::getRAreaNumberHistrogram();
	}
	
	public static function getRmax($shapePointList){
		
		return sqrt(pow((ShapePoint_ComparerNeutralPoint::getXMax($shapePointList)-
					ShapePoint_ComparerNeutralPoint::getXMin($shapePointList)
							),2)
					+pow((ShapePoint_ComparerNeutralPoint::getYMax($shapePointList)-
					ShapePoint_ComparerNeutralPoint::getYMin($shapePointList)
							),2)
					);
	}
